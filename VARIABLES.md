...
> ### ✅ 보류 해제 — 3차 패스 v2y 로 확정 (2026-08-04)
>
> 위 조건("2m 습도·PWAT 가 실제로 기여하는지")을 측정으로 확인했다
> (`기상수치모델/스케줄링/V2_실험결과.md` §11~§14):
>
> - PWAT·TCDC 를 **시잉 피처로 넣어도 무익** (F3 위에서 Δr=+0.001, p=0.398) — §12
> - 2m RH 는 기상탑과 편향 +61.8%p. 원인은 **v12 격자면이 764 m** 라 사이트(2,635 m)보다
>   **1,871 m 낮은 것** — §14. MOS 로 보정해도 정상 고도 습도를 대리변수로만 본다
> - 같은 이유로 **지표 풍속도 무력**(r=−0.03)했는데, **750 hPa 층 바람은 r=0.79** 였다.
>   층을 쓰면 격자 지형 문제가 풀린다는 것이 실증됐다 — 습도도 같은 처방이다
>
> **받는 것 (worker_v2y.py)**
>
> | 변수 | 층 | MB/단위 | 비고 |
> |---|---|---:|---|
> | **SPFH** (비습) | 31층 전부 | 5.87 | RH 대신. 층기온이 이미 있어 RH 는 파생 가능하고, 비습은 고도차에 안 흔들린다 |
> | **TCDC** 저·중·고·경계층 | 4종 | 0.53 | 사용자 지시 "상중하 나눠서". 권운=투과율만 · 층운=완전차단 · 경계층운=발밑일 수 있다 |
> | **TCDC** 전 대기 (0.5°) | — | 0.16 | 층별 검증 기준선. 기존 tcdc 는 0.25° 라 **격자점이 다르다** |
>
> RH 는 받지 않는다 (SPFH + 층기온으로 파생). 대류운·475 mb 도 뺀다.
> 작업 목록은 **v2 가 이미 받은 (cycle,lead)** 에서 만든다 — 조인이 100% 맞게.
